...
class Pet {
  ...
  Pet copyWith({int? id, String? name, PetSpecies? species, PetGender? gender,
    DateTime? birthDate, bool clearBirth=false, DateTime? welcomeDate, bool clearWelcome=false,
    DateTime? passedDate, bool clearPassed=false, String? profilePhotoPath, bool clearPhoto=false, String? memo}) =>
  ...
  factory Pet.fromMap(Map<String,dynamic> m) => Pet(id:m['id'],name:m['name'],
    ...
    profilePhotoPath:m['profilePhotoPath'],memo:m['memo']??'',
...
class DiaryEntry {
  final int? id; final int petId; final DateTime date; final DiaryMood mood;
  final String body; final List<String> photoUris; final DateTime createdAt,updatedAt;
  ...
  DiaryEntry copyWith({DiaryMood? mood,String? body,List<String>? photoUris}) =>
    DiaryEntry(id:id,petId:petId,date:date,mood:mood??this.mood,body:body??this.body,
  ...
  factory DiaryEntry.fromMap(Map<String,dynamic> m) => DiaryEntry(id:m['id'],petId:m['petId'],
...
// 予定
class PetSchedule {
  final int? id; final int? petId; final DateTime date; final String title; final String? note; final bool notifyEnabled; final DateTime createdAt;
  const PetSchedule({this.id,this.petId,required this.date,required this.title,this.note,this.notifyEnabled=true,required this.createdAt});
  PetSchedule copyWith({String? title,String? note,bool? notifyEnabled}) =>
    PetSchedule(id:id,petId:petId,date:date,title:title??this.title,note:note??this.note,notifyEnabled:notifyEnabled??this.notifyEnabled,createdAt:createdAt);
  Map<String,dynamic> toMap() => {'id':id,'petId':petId,'date':date.millisecondsSinceEpoch,'title':title,'note':note,'notifyEnabled':notifyEnabled?1:0,'createdAt':createdAt.millisecondsSinceEpoch};
  factory PetSchedule.fromMap(Map<String,dynamic> m) => PetSchedule(id:m['id'],petId:m['petId'],
    date:DateTime.fromMillisecondsSinceEpoch(m['date']),title:m['title'],note:m['note'],
    notifyEnabled:(m['notifyEnabled']??1)==1,createdAt:DateTime.fromMillisecondsSinceEpoch(m['createdAt']));
}
...
class Expense {
  final int? id; final int? petId; final DateTime date; final ExpenseCategory category; final int amount; final String memo; final DateTime createdAt;
