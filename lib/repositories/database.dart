...
class AppDatabase {
  ...
  Future<Database> _initDb() async {
    ...
    return openDatabase(join(p,'petlog.db'), version:4,
      ...
      onUpgrade:(db,old,nw) async {
        ...
        if(old<3){ try{await db.execute('CREATE TABLE IF NOT EXISTS schedules (id INTEGER PRIMARY KEY AUTOINCREMENT, petId INTEGER, date INTEGER NOT NULL, title TEXT NOT NULL, note TEXT, notifyEnabled INTEGER DEFAULT 1, notifyMinutesBefore INTEGER DEFAULT 30, createdAt INTEGER NOT NULL)');}catch(_){} }
        if(old<4){ try{await db.execute('ALTER TABLE schedules ADD COLUMN notifyMinutesBefore INTEGER DEFAULT 30');}catch(_){} }
      });
  }
  ...
  Future<void> _createAll(Database db) async {
    ...
    await db.execute('CREATE TABLE schedules (id INTEGER PRIMARY KEY AUTOINCREMENT, petId INTEGER, date INTEGER NOT NULL, title TEXT NOT NULL, note TEXT, notifyEnabled INTEGER DEFAULT 1, notifyMinutesBefore INTEGER DEFAULT 30, createdAt INTEGER NOT NULL)');
  }
  ...
  Future<List<DiaryEntry>> getEntriesByPet(int petId) async =>
    (await (await db).query('diary_entries',where:'petId=? OR petId=-1',whereArgs:[petId],orderBy:'date DESC')).map(DiaryEntry.fromMap).toList();
  Future<List<DiaryEntry>> getAllEntries() async =>
    (await (await db).query('diary_entries',orderBy:'date DESC')).map(DiaryEntry.fromMap).toList();
  Future<DiaryEntry?> getLatestEntryForPet(int petId) async {
    final r=await (await db).query('diary_entries',where:'petId=? OR petId=-1',whereArgs:[petId],orderBy:'date DESC',limit:1);
    return r.isEmpty?null:DiaryEntry.fromMap(r.first);
  }
  Future<DiaryEntry?> getRandomEntryForPet(int petId) async {
    final r=await (await db).query('diary_entries',where:'petId=? OR petId=-1',whereArgs:[petId],orderBy:'RANDOM()',limit:1);
    return r.isEmpty?null:DiaryEntry.fromMap(r.first);
  }
