...
class _ExpenseScreenState extends State<ExpenseScreen> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) => context.read<AppProvider>().loadExpenses());
  }
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _MonthSwitcher(
                    year: provider.expenseYear,
                    month: provider.expenseMonth,
                    onPrev: () => provider.navigateExpenseMonth(-1),
                    onNext: () => provider.navigateExpenseMonth(1),
                  ),
                ],
              ),
            ),
          ...
          // 月別比較
          SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4), child: _CompareCard(provider: provider))),
          ...
          if (provider.expenses.isEmpty)
            const SliverToBoxAdapter(
              child: Padding(
                ...
                child: Center(child: Text('まだ支出の記録がありません', style: TextStyle(color: AppColors.textLight))),
              ),
            )
          else
            SliverList(
          ...
          if (!provider.isPro) const SliverToBoxAdapter(child: BannerAdPlaceholder()),
          ...
          SizedBox(
            ...
            child: total > 0
                ...
                : const Center(child: Column(
                    ...
                    children: [Text('¥0', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: AppColors.textDark)), Text('今月合計', style: TextStyle(fontSize: 10, color: AppColors.textLight))],
                  )),
          ),
          if (total > 0) ...[
...
class _CompareCard extends StatelessWidget {
  final AppProvider provider;
  const _CompareCard({required this.provider});

  @override
  Widget build(BuildContext context) {
    // 仮のダミーデータ（実際はDBから過去月を取得）
    final current = provider.expenseTotal;
    return PetoCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          ...[(provider.expenseMonth - 2, current ~/ 3), (provider.expenseMonth - 1, current * 2 ~/ 3), (provider.expenseMonth, current)].map((pair) {
            final m = pair.$1 <= 0 ? pair.$1 + 12 : pair.$1;
            final val = pair.$2;
            final isCurrent = m == provider.expenseMonth;
            return Padding(
              padding: const EdgeInsets.only(bottom: 8),
              child: Row(
                children: [
                  SizedBox(width: 32, child: Text('$m月', style: TextStyle(fontSize: 11, color: isCurrent ? AppColors.caramel : AppColors.textLight, fontWeight: isCurrent ? FontWeight.bold : FontWeight.normal))),
                  Expanded(
                    child: ClipRRect(
                      borderRadius: BorderRadius.circular(10),
                      child: LinearProgressIndicator(
                        value: current > 0 ? val / (current + 1) : 0,
                        minHeight: 20,
                  ...
                  SizedBox(width: 58, child: Text('¥${_fmt(val)}', style: TextStyle(fontSize: 11, fontWeight: isCurrent ? FontWeight.w900 : FontWeight.normal, color: isCurrent ? AppColors.caramel : AppColors.textDark), textAlign: TextAlign.end)),
                ],
              ),
            );
          }),
      ...
      child: PetoCard(
        child: Row(
          children: [
            Container(
              width: 40, height: 40,
              decoration: BoxDecoration(color: Color(expense.category.colorValue).withOpacity(0.12), borderRadius: BorderRadius.circular(12)),
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              children: ExpenseCategory.values.map((cat) => FilterChip(
            ...
            TextField(
              ...
              decoration: const InputDecoration(hintText: '例：3500'),
            ),
            const SizedBox(height: 14),
